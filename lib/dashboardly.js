...
const BOOKMARK_WRITE_FIELDS = ['boardId', 'title', 'url', 'description'];
const BOOKMARK_FIELDS = ['id', 'boardId', 'title', 'url', 'description', 'createdAt', 'updatedAt'];
...
class DashboardlyDataLoader {
  ...
  getUserFromSession(sessionToken) {
    ...
    .then((result) => {
      if (result.length === 1) {
        ...
        // added avatarURL property to user object returned by query, this method hashes the email address and returns a Gravatar image url to the auth/me
        result[0].avatarURL = 'https://www.gravatar.com/avatar/' + md5(result[0].users_email.toLowerCase()) + '?d=https://www.gravatar.com/avatar/00000000000000000000000000000000';
  ...
  // Delete a session (logout) AUTH 3
  deleteToken(token) {
    return this.query(
      knex
      .delete()
  ...
  boardBelongsToUser(boardId, userId) {
    ...
    throw new Error('Board Does Not Belong To You - Access denied');
  });
  }
...
bookmarkBelongsToUser(bookmarkId, userId) {
    ...
    console.log("userId in bbtuser2",userId)

    return this.query(
      knex
        .select(util.joinKeys('boards', BOARD_FIELDS))
        .from('boards')
        .join('bookmarks', 'boards.id', '=', 'bookmarks.boardId')
        .where({
      ...
      .then((results) => {
      console.log("OwnerID",results[0].boards_ownerId)
  console.log("Results",results)


      if (results[0].boards_ownerId === userId) {
      return results[0].boards_id;
    }

    throw new Error('Bookmark not yours - Access denied');
  });
  }
