...
// old arguments were bookmarkBelongsToUser(bookmarkId, userId) but changed to bookmarkId, userData
...
// NEED A JOIN HERE ************
    return this.query(
      knex
        .select(util.joinKeys('bookmarks', BOOKMARK_FIELDS))
        .from('bookmarks')
...
// old arguments were updateBookmark(bookmarkData) but changed to (bookmarkData, userData)
...
//Matt
// added USER DATA here too

  deleteBookmark(bookmarkId, userData) {
    // TODO: this is up to you to implement :)
  const errors = validate.user(userData);
  if (errors) {
    return Promise.reject({ errors: errors });
